...
export function buildGenerationPrompt(
  ...
  const cameraInstruction: Record<string, string> = {
    wide: "远景布局照：相机退到房间入口、角落或二层俯视边缘，呈现房间主要建筑结构、地面、天花、墙面关系和核心家具。目标沙发占画面约 20% 到 35%，重点看摆放位置、空间尺度和动线。",
    medium: "中近景环境照：相机靠近会客区，距离比近景远一点点，使用平视或轻微侧前方机位。目标沙发占画面约 40% 到 60%，不要求完整展示沙发，允许裁掉少量边缘；同时看得到茶几、地毯、窗帘、灯具或墙面等周边环境。",
    close: "近景产品细节照：相机贴近沙发正前方、侧前方或略从背侧切入，画面只需要展示沙发的一部分产品细节，例如扶手、靠背、坐垫、皮纹/布纹、缝线、抱枕或转角结构。允许明显局部裁切沙发，只保留少量窗景、茶几、地面或灯具作为氛围背景。"
  };
  ...
  return [
    ...
    "原则排序：产品一致性第一，摆放方案第二，视角变化第三，画面美观第四。为了换视角不得重新设计沙发，不得把沙发换成相似款或不同座位数/不同结构的产品。",
    ...
    "多视角应像同一场景内连续拍摄：可以改变相机距离、高度、水平角度和前景/背景关系，但前提是目标沙发产品身份完全不变；如果视角变化会导致产品变形或变款，宁可让视角变化保守一点。",
    `摆放位置：${positionLabel}`,
...
export function buildCameraVariationPrompt(
  ...
  const cameraInstruction: Record<string, string> = {
    ...
    close: "近景产品细节照：相机继续靠近沙发，使用正前方、侧前方或略从背侧切入的低到中等高度机位；画面只需要展示沙发的一部分产品细节，例如扶手、靠背、坐垫、皮纹/布纹、缝线、抱枕或转角结构。允许明显局部裁切沙发，只保留少量窗景、茶几、地面或灯具作为氛围背景。"
  };
  const cameraDeltaInstruction: Record<string, string> = {
    ...
    close: "近景应比中近景更靠近产品细节，画面重点从沙发与环境转为材质和局部结构。可以局部裁切沙发；裁切是允许的，但产品细节必须仍来自同一款沙发。"
  };
  return [
    "这是同一试摆方案的换镜头任务。第一张输入图是已确认的远景主图；第二张是沙发前景；第三张是用户原始沙发产品参考图。",
    "保持第一张图中的房间、摆放位置、沙发尺度、朝向、光影和主要家具关系不变，在此基础上调整相机距离与构图。",
    "最重要原则：最终图里的沙发必须继续像第三张产品参考图。产品一致性高于视角变化；为了换镜头不得重新设计沙发，不得把它改成相似款，不得改变贵妃位/转角方向、模块数量、扶手、靠背、坐垫分区、主色和材质。",
    "视角变化可以保守；允许中近景或近景包含局部裁切。禁止的是把沙发变成另一款产品，而不是禁止合理裁切。",
    buildHumanModelPrompt(settings),
...
export function buildVirtualRoomPrompt(
  ...
  const cameraInstruction: Record<string, string> = {
    ...
    close: "近景产品细节照：相机贴近沙发正前方、侧前方或略从背侧切入；画面只需要展示沙发的一部分产品细节，例如扶手、靠背、坐垫、材质纹理、缝线、抱枕或转角结构。允许明显局部裁切沙发，仅保留少量虚拟房间上下文。"
  };
  ...
  return [
    ...
    "原则排序：产品一致性第一，虚拟房间合理性第二，视角变化第三，画面美观第四。为了换视角不得重新设计沙发，不得把沙发换成相似款或不同座位数/不同结构的产品。",
    ...
    "多视角生成时，同一个沙发、同一个房间、同一个摆放方案必须保持一致。可以改变相机距离、构图和局部裁切，但前提是目标沙发产品身份完全不变；如果视角变化会导致产品变形或变款，宁可让视角变化保守一点。",
    extraContext ? `平台传入上下文：${extraContext}` : "",
    extraPrompt.length ? `平台补充关键词：${extraPrompt.join("、")}` : ""
  ].filter(Boolean).join("\n");
}
