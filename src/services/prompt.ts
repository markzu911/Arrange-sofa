...
export function buildCameraVariationPrompt(
  ...
  const cameraInstruction: Record<string, string> = {
    ...
    close: "近景：相机贴近目标沙发正前方或侧前方，目标沙发占画面约 65% 到 80%，清晰展示扶手、靠背、坐垫和材质细节；只保留局部地面、墙面或茶几作为环境上下文。"
  };
  ...
  return [
    ...
    "禁止输出仅裁切、缩放或放大主图得到的画面。如果无法完成真实机位重构，也必须生成一个明显更近、更偏侧、更聚焦沙发的镜头，而不是返回与远景几乎相同的构图。",
