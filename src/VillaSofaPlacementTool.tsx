...
import { analyzeScene, generatePlacementImages, generateVirtualRoomImages } from "./services/gemini";
import { compressDataUrlToBlob, compressImage, GEMINI_IMAGE_TARGET_BYTES } from "./services/image";
import { buildGenerationPrompt, buildVirtualRoomPrompt } from "./services/prompt";
import {
  consumeIntegral,
...
export function VillaSofaPlacementTool() {
  ...
  const previewPrompt = useMemo(() => {
    if (!analysis) return "";
    const perspective = settings.perspectives[0] || "medium";
    return useVirtualRoom
      ? buildVirtualRoomPrompt(analysis, settings, perspective, platform.context, platform.prompt)
      : buildGenerationPrompt(analysis, settings, perspective, platform.context, platform.prompt);
  }, [analysis, settings, useVirtualRoom, platform.context, platform.prompt]);
  ...
  const guideCopy = useMemo(() => {
    ...
    if (guidedStep === "generating") {
      return {
        eyebrow: "第 4 步",
        title: "正在生成试摆效果",
        desc: "我正在按已确认的方案生成图片，完成后自动进入结果页。",
        hint: "当前为单视角生成，请保持页面打开直到生成结束。"
      };
    }
    return {
      eyebrow: "完成",
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const images = useVirtualRoom
        ? await generateVirtualRoomImages(sofaImage, productReferenceImage, analysis, settings, platform.context, platform.prompt)
        : await generatePlacementImages(roomImage as UploadedImage, sofaImage, productReferenceImage, [], analysis, settings, platform.context, platform.prompt);

      if (images.length !== settings.perspectives.length) {
        throw new Error(`视角结果不完整：已选择 ${settings.perspectives.length} 个视角，但仅生成 ${images.length} 张图片。请重新生成。`);
      }
      ...
      addChatMessage({ role: "assistant", text: `试摆效果已经生成。您可以在下方查看效果，或返回调整换一个视角重新生成。` });
        ...
        {substep === "plan" ? (
          ...
          <>
              ...
              <div className={styles.optionBlock}>
                ...
                <div className={styles.choiceGrid}>
                  {Object.entries(perspectiveLabels).map(([key, label]) => (
                    <button key={key} className={settings.perspectives[0] === key ? styles.selectedChoice : ""} onClick={() => onPerspectiveSelect(key as never)} aria-pressed={settings.perspectives[0] === key}>
                      {label}
                    </button>
                  ))}
                </div>
            ...
            <div className={styles.foldActions}>
              <button className={styles.secondaryButton} onClick={() => setShowPrompt((value) => !value)}>
                <PenLine size={16} />
            ...
            {showPrompt && previewPrompt && (
              <div className={styles.analysisEditor}>
                <div className={styles.optionHeading}>
                  ...
                  <span>以下文本连同房间图和沙发图一起发送给 Gemini</span>
                </div>
                <textarea readOnly value={previewPrompt} className={styles.promptPreviewArea} />
              </div>
            )}
